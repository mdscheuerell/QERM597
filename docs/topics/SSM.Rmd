...
## Examining model fits in detail {.smaller}

```{r brw_MARSS_output_detail_NLL, eval = FALSE, echo = TRUE}
...
```{r brw_MARSS_output_detail_pars, eval = FALSE, echo = TRUE}
## Success! abstol and log-log tests passed at 61 iterations.
## Alert: conv.test.slope.tol is 0.5.
## Test with smaller values (<0.1) to ensure convergence.
## 
...
Fitted values are stored in a $1 \times T$ matrix (vector) accessed with `$states`

Standard errors of fitted values are stored in a $1 \times T$ matrix (vector) accessed with `$states.se`
...
We can estimate a $(1 - \alpha/2)\%$ confidence interval as
...
## Plot the model fit without true state
...
What is the expectation of $x_t$?
...
## Density-dependent population growth | Estimating parameters
...
```{r MARSS_output_detail_NLL, eval = FALSE, echo = TRUE}
## Success! abstol and log-log tests passed at 61 iterations.
## Alert: conv.test.slope.tol is 0.5.
## Test with smaller values (<0.1) to ensure convergence.
## 
...
```{r MARSS_output_detail_pars, eval = FALSE, echo = TRUE}
## Success! abstol and log-log tests passed at 61 iterations.
## Alert: conv.test.slope.tol is 0.5.
## Test with smaller values (<0.1) to ensure convergence.
## 
